 ...
 */
class FileTree extends javafx.scene.control.TreeView<java.io.File> {    //javafx.scene.control.TreeView a lenyíló listát teszi lehetővé

    class FileTreeItem extends javafx.scene.control.TreeItem<java.io.File> {    //lenyíló lista eleme lesz
        protected boolean leaf = true;

        public FileTreeItem(java.io.File file, javafx.scene.Node icon) {    //konstruktor file-lal és ikonnal
            super(file, icon);      //ősosztály konstruktora
            leaf = file.isFile();   //ha a bemenő file fájl akk igaz
        }
        ...
        public javafx.collections.ObservableList<javafx.scene.control.TreeItem<java.io.File>> getChildren() {   //olyan lista, amelynek a változásait nyomon lehet követni a változás beköetkezésekor
            ...
            return super.getChildren();     //getchildren visszaadja a kollekció elemeit
        }

        private javafx.collections.ObservableList<javafx.scene.control.TreeItem<java.io.File>>buildChildren(javafx.scene.control.TreeItem<java.io.File> treeItem) {
    ...
    public void save(javafx.scene.control.TextArea propsEdit, javafx.scene.control.Label actPropsLabel) {   //szöveg mentése
    ...
    public FileTree(String cityName, boolean expanded, javafx.scene.control.TextArea propsEdit, javafx.scene.control.Label actPropsLabel) {
        ...
        setRoot(root);
        setCellFactory((javafx.scene.control.TreeView<java.io.File> p) -> new TextFieldTreeCell(propsEdit)); //szerkeszthető nevű fa elem

        setOnMouseClicked((javafx.scene.input.MouseEvent evt) -> { //egérkattintást figyeli
            if (evt.getClickCount() == 1) {     //hányszor lett megnyomva
                javafx.scene.control.TreeItem<java.io.File> item = getSelectionModel().getSelectedItem(); //ha egyszer megnézi mi van kiválasztva
                if (item != null) {
                    ...
                    try {
                        ...
                        while (scanner.hasNextLine()) {
                            fileContents.append(scanner.nextLine());        //beolvassa soronként és összefűzi
                            fileContents.append(System.getProperty("line.separator"));  //rendszerváltozóban a sorvégjel
                        }
                        scanner.close();
                        save(propsEdit, actPropsLabel);     //elmenti a textarea és a label-t
    ...
    private final class TextFieldTreeCell extends javafx.scene.control.TreeCell<java.io.File> {
        ...
        public TextFieldTreeCell(javafx.scene.control.TextArea propsEdit) { //jobbklikk események
            ...
            subaMenuItem.setOnAction((javafx.event.ActionEvent evt) -> {
                ...
                java.io.File f = new java.io.File(file.getPath() + System.getProperty("file.separator") + "uj");
...
//                        = new javafx.scene.control.TreeItem<java.io.File>(f, new javafx.scene.image.ImageView(actpropsIcon));
        ...
        private void editCell() {       //a tevékenységfa elemeinek szerkesztése
            if (getItem() == null) {
                return;
            }
...
class StringTree extends javafx.scene.control.TreeView<String> {
    ...
    protected final javafx.scene.image.Image propIcon = new javafx.scene.image.Image(getClass().getResourceAsStream("props.png"));
    protected final javafx.scene.image.Image actIcon = new javafx.scene.image.Image(getClass().getResourceAsStream("activity.png"));
    protected final javafx.scene.image.Image actpropsIcon = new javafx.scene.image.Image(getClass().getResourceAsStream("activityprops.png"));
    javafx.scene.control.TextArea propsEdit;
    ...
    public StringTree(java.util.List<String> properties, boolean expanded, javafx.scene.control.TextArea propsEdit) {
        ...
        setOnMouseClicked((javafx.scene.input.MouseEvent mouseEvent) -> {   //textarea-ba ír
            {
...
// mappa név szerkesztése esetén ne uj mappát hozzon létre, hanem megvizsgálni, hogy létezik e alkönyvtár és akk átnevezni